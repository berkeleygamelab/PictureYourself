// This might not factory (possibly a service or just a function)
app.factory('Sticker', function(){
	return{
		new : function(imageObj, pos, start_size, layer, imageObjBack){
    		var sticker = {
    			background : null,
				delete_icon : null,
				group : null,
				image : null,
				imageBack : null,
				move_color : null,
				previous_color : null,
				rotate : null,
				scalerX : null,
				scalerY : null, 
				reposition : null
			}

	        // Used to make sure both background and foreground are loaded together
			var image_load_count = 0;
			// Used to make sure that scaling and positioning of controls are synced
			var tool_size = 30;

	        sticker.group = new Kinetic.Group({
	            draggable: true,
	            x: pos.x + start_size.width/2,
	            y: pos.y + start_size.height/2
	        });

	        console.log(pos.x + start_size.width/2, pos.y + start_size.height/2);

            sticker.image = new Kinetic.Image({
               image:imageObj,
               width: start_size.width,  //this makes the image lower quality for some reason
               height: start_size.height,
               x: 0, // pos.x + start_size.width/2,
               y: 0, // pos.y + start_size.height/2,
               offsetX: start_size.width/2,
               offsetY: start_size.height/2,
               stroke:'green'
            });

		   	if (imageObjBack != null){
	            sticker.imageBack = new Kinetic.Image({
	               image:imageObjBack,
	               width: start_size.width,  //this makes the image lower quality for some reason
	               height: start_size.height,
	               offsetX: start_size.width/2,
               	   offsetY: start_size.height/2,
	               x: 0, // pos.x + start_size.width/2,
	               y: 0, // pos.y + start_size.height/2,
	        	});
	        }

	       	sticker.delete_icon = new Kinetic.Text({
	            visible:true,
	            text: '',
	            fontFamily: 'FontAwesome',
	            fontSize: tool_size,
	            fill: '#eee',
	            stroke: "#222",
	            strokeWidth: 2,
	            name: 'delete', 
	            x: start_size.width/2,
	            y: -start_size.height/2,
	            offsetX: tool_size/2,
	            offsetY: tool_size/2
			});

			sticker.scalerX = new Kinetic.Text({
	            x : start_size.width/2, // sticker.image.x() + start_size.width/2,
	            y : 0, // sticker.image.y(),
	            offsetX: tool_size/2,
	            offsetY: tool_size/2,
	            text: '',
	            fontFamily: 'FontAwesome',
	            fontSize: tool_size,
	            fill: '#eee',
	            stroke: "#222",
	            strokeWidth: 2,
	            draggable:true,
	            visible:true,
	            name: 'x',
	            // dragBoundFunc: function(pos){
	            //     return{
	            //         x: pos.x,
	            //         y: this.getAbsolutePosition().y
	            //     };
	            // }
	        });

            sticker.scalerY = new Kinetic.Text({
	            x : 0, // sticker.image.x(),
	            y : start_size.height/2, // sticker.image.y() + start_size.height/2,
	            offsetY: tool_size/2,
	            offsetX: tool_size * 13 / 60,
	            text: '',
	            fontFamily: 'FontAwesome',
	            fontSize: tool_size,
	            fill: '#eee',
	            stroke: "#222",
	            strokeWidth: 2,
	            draggable:true,
	            visible:true,
	            name: 'y',
	            //offset:[image.getWidth()/2,image.getHeight()/2],
	            // dragBoundFunc: function(pos){
	            //   return{
	            //     x: this.getAbsolutePosition().x,
	            //     y: pos.y
	            //   };
	            // }
	        });

            sticker.rotate = new Kinetic.Text({
	            x : - start_size.width/2, // sticker.image.x() - start_size.width/2,
	            y : - start_size.height/2, // sticker.image.y() - start_size.height/2,
	            offsetX: tool_size/2,
	            offsetY: tool_size/2,
	            text: '',  //leave this it won't render correctly here but will on the canvas
	            fontFamily: 'FontAwesome',
	            fontSize: tool_size,
	            fill: '#eee',
	            stroke: "#222",
	            strokeWidth: 2,
	            draggable:true,
	            visible:true,
	            name: 'rotate',
	            // dragBoundFunc: function(pos) {
	            //     var offX = pos.x - sticker.image.getAbsolutePosition().x;
	            //     var offY = pos.y - sticker.image.getAbsolutePosition().y;  // your offset
	            //     var radius = Math.sqrt(Math.pow(sticker.image.getWidth()/2,2) + Math.pow(sticker.image.getHeight()/2,2));//60;//Math.min(sticker.image.getWidth() / 2 , sticker.image.getHeight() / 2);//60;
	            //     var scale = radius / Math.sqrt(Math.pow(offX, 2) + Math.pow(offY, 2)); // distance formula ratio
	            //     return {
	            //         y: (offX * scale + sticker.image.getAbsolutePosition().y),
	            //         x: (offY * scale + sticker.image.getAbsolutePosition().x)
	            //     };

	            // }
	        });

			// ▼▼ REMOVE WHEN ROTATE IS COMPELTED ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
			// sticker.rotate.setVisible(false);
			// ▲▲ REMOVE WHEN ROTATE IS COMPELTED ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲

	        if (imageObjBack != null){
	            sticker.imageBack.offsetX(start_size.width/2);
	            sticker.imageBack.offsetY(start_size.height/2);
	        }

	        if(imageObjBack != null){
	            // Using image_load_count as a counter to make sure
	            // both the background and foreground are loaded before onload process

	            imageObjBack.onload = function(){
	                if(image_load_count > 0){
	                    return load();
	                }

	                image_load_count++;
	            };

	            imageObj.onload = function(){
	                if(image_load_count > 0){
	                    return load();
	                };
	                image_load_count++;
	            };

	        }

	        else{
	            imageObj.onload = function(){
	                return load();
	            };
	        }

	        //construct group to drop after image loads
	        var load = function(){
	            
	            if(imageObjBack != null){
	                sticker.group.add(sticker.imageBack);
	                sticker.move_color();
	            }
	            sticker.group.add(sticker.image);
	            sticker.group.add(sticker.scalerX);
	            sticker.group.add(sticker.scalerY);
	            sticker.group.add(sticker.delete_icon);
	            sticker.group.add(sticker.rotate);

	            layer.add(sticker.group);
	            
	            sticker.reposition();
	            layer.draw();
	        }

			sticker.reposition = function(){
		        // var x = sticker.image.getAbsolutePosition().x;
		        // var y = sticker.image.getAbsolutePosition().y;

		        // sticker.rotate.setAbsolutePosition({ x: x - sticker.image.width()/2, y: y - sticker.image.height()/2 });

		        // sticker.scalerX.setAbsolutePosition({ x: x + sticker.image.width()/2, y: y });
		        // sticker.scalerY.setAbsolutePosition({ x: x, y: y + sticker.image.height()/2 });
		        // sticker.delete_icon.setAbsolutePosition({ x: x + sticker.image.width()/2, y: y  - sticker.image.height()/2 });
		        // sticker.rotate.setAbsolutePosition({ x: -sticker.image.width()/2, y: -sticker.image.height()/2 });
		        sticker.rotate.x(-sticker.image.width()/2);
		        sticker.rotate.y(-sticker.image.height()/2);
		        // sticker.scalerX.setAbsolutePosition({ x: sticker.image.width()/2, y: 0 });
		        sticker.scalerX.x(sticker.image.width()/2);
		        sticker.scalerX.y(0);
		        // sticker.scalerY.setAbsolutePosition({ x: 0, y: sticker.image.height()/2 });
		        sticker.scalerY.x(0);
		        sticker.scalerY.y(sticker.image.height()/2);
		        // sticker.delete_icon.setAbsolutePosition({ x: sticker.image.width()/2, y: -sticker.image.height()/2 });
		        sticker.delete_icon.x(sticker.image.width()/2);
		        sticker.delete_icon.y(-sticker.image.height()/2);

	        };

	        sticker.move_color = function(){

	            var y = sticker.scalerY.getAbsolutePosition().y - sticker.image.height()/2;
	            var x = sticker.scalerX.getAbsolutePosition().x  
	                 + $('#container').offset().left
	                 - sticker.image.getWidth() 
	                 - sticker.image.offsetX() 
	                 - $('#modal').width();

	            $("#modal").css({left: x, top: y});
	            $("#modal").show();
	        }

	 		return sticker;
	     
		}
	}
});
